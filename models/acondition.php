...
class Acondition extends AdmObject
{
        ...
        {
                $allFArr = $this->getAllFields(null, $what);
                ...
                if ($this->id == $acond_id_of_sis_fields_available) {
                        ...
                        if ($afObj and ($afObj->id > 0)) {
                                if ($what != "object") 
                                {
                                        ...
                                        $return_arr[$afObj->id] = $afObj;
                                }


                                for ($kf = 1; $kf <= 3; $kf++) {
                                        $afFormula_kf_Obj = $afObj->het("formula_field_" . $kf . "_id");
                                        if ($afFormula_kf_Obj and ($afFormula_kf_Obj->id > 0)) {
                                                if ($what != "object") 
                                                {
                                                        $return_arr[] = ["id"=>$afFormula_kf_Obj->id, "name"=>$afFormula_kf_Obj->getDisplay("en")];
                                                } else {
                                                        $return_arr[$afFormula_kf_Obj->id] = $afFormula_kf_Obj;
                                                }
                                        }
                                }
                        }
        ...
        protected function getOtherLinksArray($mode, $genereLog = false, $step = "all")
        {
                ...
                if ($my_id and ($mode == "mode_condition_2_id") and $this->_isComposed())/* and (
