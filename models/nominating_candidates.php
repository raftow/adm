...
class NominatingCandidates extends AdmObject
{
    ...
    {
        if (!$nomination_letter_id)
            throw new AfwRuntimeException('loadByMainIndex : nomination_letter_id is mandatory field');
        if (!$identity_type_id)
            throw new AfwRuntimeException('loadByMainIndex : identity_type_id is mandatory field');
        if (!$idn)
            throw new AfwRuntimeException('loadByMainIndex : idn is mandatory field');
    ...
    {
        if (!$applicant_id)
            throw new AfwRuntimeException('loadByApplicationInfos : applicant_id is mandatory field');
        if (!$application_plan_id)
            throw new AfwRuntimeException('loadByApplicationInfos : application_plan_id is mandatory field');
        if (!$application_simulation_id)
            throw new AfwRuntimeException('loadByApplicationInfos : application_simulation_id is mandatory field');
    ...
    public function beforeMaj($id, $fields_updated)
    {
        ...
        if ($fields_updated['academic_program_id']) {
            ...
            if (!$fields_updated['application_plan_branch_mfk'])
                $fields_updated['application_plan_branch_mfk'] = '@wasEmpty';
    ...
    public function beforeDelete($id, $id_replace)
    {
        ...
        if ($id) {
            ...
            if ($application_plan_id and $applicant_id and $application_simulation_id) {
                $objme = AfwSession::getUserConnected();
                if (!$objme)
                    $this->deleteNotAllowedReason = 'Please authenticate before';
                ...
                if ($this->deleteNotAllowedReason)
                    return false;
    ...
    public function getMyApplication()
    {
        if (!$this->applicationObj) {
            ...
            if ($applicant_id and $application_plan_id and $application_simulation_id) {
                ...
                if ($this->getVal('application_plan_branch_mfk'))
                    $this->applicationObj->set('application_plan_branch_mfk', $this->getVal('application_plan_branch_mfk'));
    ...
    public function calcCandidateInfo($what = 'value')
    {
        ...
        $paymentType = $this->decode('study_funding_status_id', '', false, $lang);
    ...
    public function calcMyApplicationLink($what = 'value')
    {
        ...
        if (!$applicantObj) {
            ...
            if (!$applicantObj or !$applicant_id)
                throw new AfwRuntimeException('failed to create applicant profile record');
        } else
    ...
    public function calcApplyConditionsDiv($what = 'value')
    {
        ...
        if ($applicationObj) {
            ...
            $html = 'No application found, please contact administartor';
    ...
    public function calcDesiresLimitWarningDiv($what = 'value')
    {
        ...
        if ($mfk)
            $branch_count = count(explode(',', $mfk));
        else
            $branch_count = 0;
    ...
    public function calcTrackOverpassDiv($what = 'value')
    {
        ...
        if ($this->sureIs('track_overpass')) {
            ...
            if (!$rack_overpass_when)
                $rack_overpass_when = 'غير معروف';
    ...
    public function calcRatingOverpassDiv($what = 'value')
    {
        ...
        if ($this->sureIs('rating_overpass')) {
            ...
            if (!$rating_overpass_when)
                $rating_overpass_when = 'غير معروف';
